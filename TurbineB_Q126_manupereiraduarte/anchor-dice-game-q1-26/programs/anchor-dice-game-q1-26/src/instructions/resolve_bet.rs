...
pub struct ResolveBet<'info> {
    ...
    #[account(mut)]
    /// CHECK: We will verify this account is the player by checking the bet account's has_one constraint
    ...
    /// CHECK: This is safe because we only read from this account and verify the program ID and data ourselves
...
impl<'info> ResolveBet<'info> {
    pub fn verify_ed25519_signature(&mut self, sig: &[u8]) -> Result<()> {
        let ix = load_instruction_at_checked(0, 
    ...
    pub fn resolve_bet(&mut self, bumps: &ResolveBetBumps, sig: &[u8]) -> Result<()> {
        ...
        if self.bet.roll > roll {
            require!(self.bet.roll > 1, DiceError::MinimumRoll);
